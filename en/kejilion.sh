		...
		case $choice in
			1)
				...
				while true; do
					...
					if ss -tuln | grep -q ":$app_port "; then
						echo -e "${gl_hong}mistake:${gl_bai}端口 $app_port 已被占用，请更换一个端口"
						send_stats "应用端口已被占用"
					else
						local docker_port=$app_port
						break
...
list_stream_services() {
	...
	printf "%-25s %-18s %-25s %-20s\n" "Service name" "通信类型" "本机地址" "Backend address"
...
list_forwarding_services() {
	...
	# 打印表头
	printf "%-20s %-25s %-30s %-10s\n" "Service name" "内网地址" "外网地址" "protocol"
...
bbrv3() {
		  ...
		  xanmod_add_repo() {
				...
				if echo "jammy focal bullseye buster" | grep -qw "$os_codename" || [ "$os_codename" = "releases" ]; then
					echo -e "${gl_hong}XanMod 官方已停止对当前系统($os_codename), please upgrade to Debian12/Ubuntu24 or higher.${gl_bai}"
					return 1
				fi
...
openclaw_detect_api_protocol_by_provider() {
	...
	# 保留函数以兼容菜单调用，但不做任何改写。
	echo "ℹ️ 已关闭协议探测：请手动在 ${HOME}Set provider.api in /.openclaw/openclaw.json to openai-completions or openai-responses"
	return 0
}
			...
			for token in $skill_input; do
				...
				if [ "$skill_action" = "1" ]; then
					...
					if [ -d "${HOME}/.openclaw/workspace/skills/${skill_name}" ]; then
						...
						echo "💡 技能 [$skill_name] is installed in the system directory."
						skill_found=true
					fi
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}3.   ${color3}1Panel new generation management panel${gl_kjlan}4.   ${color4}NginxProxyManager可视化面板"
	  ...
	  echo -e "${gl_kjlan}19.  ${color19}雷池WAF防火墙面板                   ${gl_kjlan}20.  ${color20}portainer container management panel"
	  echo -e "${gl_kjlan}-------------------------"
	  ...
	  echo -e "${gl_kjlan}33.  ${color33}Sun-Panel Navigation Panel${gl_kjlan}34.  ${color34}Pingvin-Share文件分享平台"
